...
/// \code
/// setTimeout(function() {
///   var objPos = Kmods.Utils.getAbsolutePos(objElement);
...
Kmods.Utils.getAbsolutePos = function(el) {
	var SL = 0, ST = 0;
...
Kmods.Utils.fixBoxPosition = function(box) {
	if (box.x < 0)
		box.x = 0;
	if (box.y < 0)
		box.y = 0;
	var cp = Kmods.Utils.createElement("div");
	var s = cp.style;
	s.position = "absolute";
	s.right = s.bottom = s.width = s.height = "0px";
	window.document.body.appendChild(cp);
	var br = Kmods.Utils.getAbsolutePos(cp);
...
///         if (!Kmods.Utils.isRelated(this, event)) {
...
Kmods.Utils.isRelated = function (el, evt) {
...
Kmods.Utils.removeClass = function(el, className) {
	if (!(el && el.className)) {
		return;
	}
...
Kmods.Utils.addClass = function(el, className) {
	Kmods.Utils.removeClass(el, className);
...
Kmods.Utils.getElement = function(ev) {
	if (Kmods.is_ie) {
...
Kmods.Utils.getTargetElement = function(ev) {
	if (Kmods.is_ie) {
		return window.event.srcElement;
	} else {
		return ev.target;
...
/// @param ev [Event] the event object
/// @return false
Kmods.Utils.stopEvent = function(ev) {
	ev || (ev = window.event);
	if (ev) {
		if (Kmods.is_ie) {
...
Kmods.Utils.addEvent = function(el, evname, func) {
...
/// Removes an event handler added with Kmods.Utils.removeEvent().  The
/// prototype scheme is the same.
Kmods.Utils.removeEvent = function(el, evname, func) {
...
Kmods.Utils.createElement = function(type, parent, selectable) {
	var el = null;
...
Kmods.Utils.writeCookie = function(name, value, domain, path, exp_days) {
	value = escape(value);
...
Kmods.Utils.getCookie = function(name) {
 ...
 */
Kmods.Utils.makePref = function(obj) {
...
/**
 * The reverse of Kmods.Utils.makePref(), this function unserializes the
 ...
 */
Kmods.Utils.loadPref = function(txt) {
	var obj = null;
	try {
		eval("obj={" + txt + "}");
	} catch(e) {}
 ...
 */
Kmods.Utils.mergeObjects = function(dest, src) {
	for (var i in src)
		dest[i] = src[i];
};
...
Kmods.Utils.__wch_id = 0;	/**< [number, static] used to create ID-s for the WCH objects */
 ...
 */
Kmods.Utils.setupWCH_el = function(f, el, el2) {
	if (f) {
		var pos = Kmods.Utils.getAbsolutePos(el),
			X1 = pos.x,
			Y1 = pos.y,
			X2 = X1 + el.offsetWidth,
			Y2 = Y1 + el.offsetHeight;
		if (el2) {
			var p2 = Kmods.Utils.getAbsolutePos(el2),
				XX1 = p2.x,
				YY1 = p2.y,
		...
		Kmods.Utils.setupWCH(f, X1, Y1, X2-X1, Y2-Y1);
	}
};
 ...
 */
Kmods.Utils.setupWCH = function(f, x, y, w, h) {
	if (f) {
		var s = f.style;
 ...
 */
Kmods.Utils.hideWCH = function(f) {
	if (f)
 ...
 *
 */
Kmods.Utils.getPageScrollY = function() {
...
// Object setup.
Kmods.ScrollWithWindow = {};
Kmods.ScrollWithWindow.list = [];
// Set to a number between 0 and 1, lower means longer scrolling.
Kmods.ScrollWithWindow.stickiness = 0.25;
 ...
 */
Kmods.ScrollWithWindow.register = function(node) {
	var top = parseInt(node.style.top) || 0;
	var scrollY = window.pageYOffset || document.body.scrollTop ||
		document.documentElement.scrollTop || 0;
	top -= scrollY;
	if (top < 0) top = 0;
	Kmods.ScrollWithWindow.list[Kmods.ScrollWithWindow.list.length] = {
		node: node,
		origTop: top
	};
};
 ...
 * @param node [HTMLElement] -- a reference to the node to scroll.
 */
Kmods.ScrollWithWindow.unregister = function(node) {
	for (var count = 0; count < Kmods.ScrollWithWindow.list.length; count++) {
		var elm = Kmods.ScrollWithWindow.list[count];
		if (node == elm.node) {
			Kmods.ScrollWithWindow.list.splice(count, 1);
			return;
		}
	}
};
 ...
 */
Kmods.ScrollWithWindow.handler = function(newScrollY) {
	...
	for (var count = 0; count < Kmods.ScrollWithWindow.list.length; count++) {
		...
		var node = elm.node;
		if (!elm.origTop) {
			elm.origTop = Kmods.Utils.getAbsolutePos(node).y;
...
var oldScrollY = Kmods.Utils.getPageScrollY();
setInterval(
	'var newScrollY = Kmods.Utils.getPageScrollY(); ' +
	'if (newScrollY != oldScrollY) { ' +
		'Kmods.ScrollWithWindow.handler(newScrollY); ' +
	'}', 50);
 ...
 */
Kmods.Utils.destroy = function(el) {
	if (el && el.parentNode)
 ...
 */
Kmods.Utils.newCenteredWindow = function(url, windowName, width, height, scrollbars){
	var leftPosition = 0;
	var topPosition = 0;
	if (screen.width)
 ...
 */
Kmods.Utils.getWindowSize = function() {
  var iWidth = 0;
  var iHeight = 0;
 ...
 */
Kmods.Utils.selectOption = function(sel, val, call_default) {
 ...
 */
Kmods.Utils.getNextSibling = function(el, tag, alternateTag) {
	el = el.nextSibling;
	if (!tag) {
		return el;
	}
...
/**
 * Similar to Kmods.Utils.getNextSibling(), this function will return the
 ...
 * found.
 */
Kmods.Utils.getFirstChild = function(el, tag, alternateTag) {
  if (!el) {
    return null;
  }
	el = el.firstChild;
  if (!el) {
	...
	return Kmods.Utils.getNextSibling(el, tag, alternateTag);
};
 ...
 */
Kmods.Utils.getChildText = function(objNode) {
	if (objNode == null) {
		return '';
	}
	var arrText = [];
 ...
 *
 */
Kmods.Utils.insertAfter = function(oldNode, newNode) {
...
Kmods.Utils._ids = {};	/**< [number, static] maintains a list of generated IDs */
 ...
 */
Kmods.Utils.generateID = function(code, id) {
	if (typeof id == "undefined") {
		if (typeof this._ids[code] == "undefined")
			this._ids[code] = 0;
		id = ++this._ids[code];
	}
	return "Kmods-" + code + "-" + id;
};
...
*/
Kmods.Utils.addTooltip = function(target, tooltip) {
return new Kmods.Tooltip(target, tooltip);
};

// Browser sniffing

/// detect Opera browser
Kmods.is_opera = /opera/i.test(navigator.userAgent);

/// detect a special case of "web browser"
Kmods.is_ie = ( /msie/i.test(navigator.userAgent) && !Kmods.is_opera );

/// detect IE5.0/Win
Kmods.is_ie5 = ( Kmods.is_ie && /msie 5\.0/i.test(navigator.userAgent) );

/// detect IE for Macintosh
Kmods.is_mac_ie = ( /msie.*mac/i.test(navigator.userAgent) && !Kmods.is_opera );

/// detect KHTML-based browsers
Kmods.is_khtml = /Konqueror|Safari|KHTML/i.test(navigator.userAgent);

/// detect Konqueror
Kmods.is_konqueror = /Konqueror/i.test(navigator.userAgent);

/// detect Gecko
Kmods.is_gecko = /Gecko/i.test(navigator.userAgent);
...
/// Kmods.Utils.Array object which contains additional for arrays method
Kmods.Utils.Array = {};
 ...
 */
Kmods.Utils.Array.insertBefore = function (arr, el, key, nextKey) {
	var tmp = new Array();
	for(var i in arr) {
		if (i == nextKey) {
			if (key) {
