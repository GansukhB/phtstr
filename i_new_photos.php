...
<table border="0" callspacing="3" width="100%">
	...
	$photocount=1;
	$new_display_limit = $setting->newest;
	...
	if($package_rows > $new_display_limit){
		$package_rows = $new_display_limit;
	}
	...
	if($pages == $page_num){
		...
		$needshown =  $new_display_limit - $shown;
		$perpage = $needshown;
	}
	$package_result = mysql_query("SELECT id,description,title,gallery_id,code FROM photo_package where active = '1' and photog_show = '1' and gallery_id IN ($approved_cats) order by id desc LIMIT $startrecord,$perpage", $db);
	while($package = mysql_fetch_object($package_result)){
		...
		// Rating system added for PS320
			...
			$rate_it = $newest_rating . round($current_rating/$vote_count, 2);
			} else {
		...
		if(file_exists($stock_photo_path . "s_" . $photo->filename)){
			...
			$no_upload_message.= "ERROR:19 Failed to display a photo in the newest photo gallery \n";
	  ...
	  // End of hover view size changing code
		...
		$new_description = $package->description;
			...
			//$the_size = getimagesize($stock_photo_path . $photos->filename);
	?>
				<?
					if($photocount == 1){
				?>
                ...
                if($photo_rows > 0){
						...
						?>
                        <?PHP echo $newest_details; ?></a><font color="#A4A4A4"><?php if($setting->dis_title_gallery == 1){ ?><? if($setting->dis_filename == 1){ echo "<br>" . $photo->filename; } else { if(trim($package->title) != ""){ echo "<br>" . $package->title; } }?><? } ?><?php if($setting->show_views == "1"){ ?><br><?PHP echo $newest_viewed; ?> <? echo $package->code; ?><? } if($setting->sr_new == 1){ ?><? echo $rate_it; } } else { ?><a href="#" class="photo_links"><?PHP echo $newest_details; ?></a><? } ?>
						<? 
										if($_SESSION['sub_member']){
												...
												if($lightbox1_rows > 0){ 
										?>
															<br /><a href="public_actions.php?pmode=remove_lightbox&lid=<? echo $lightbox1->id; ?>&cur=<? echo $cur_page; ?>" class="photo_links"><img src="images/sm_rem_lightbox.gif" border="0" alt="<?PHP echo $newest_alt_remlightbox; ?>"></a>
										<? } else { ?>
															<br /><a href="public_actions.php?pmode=add_lightbox&ptype=d&gid=<? echo $package->gallery_id; ?>&sgid=<? echo $_GET['sgid']; ?>&pid=<? echo $package->id; ?>&cur=<? echo $cur_page; ?>" class="photo_links"><img src="images/sm_add_lightbox.gif" border="0" alt="<?PHP echo $newest_alt_addlightbox; ?>"></a>
										<? } }?>
						<!--
						|
						<?
	...
	?>
		...
		<tr>
			...
			<div name="result_details" id="result_details" style="padding-left: 10px;padding-right: 10px;padding-top: 10px; padding-bottom: 10px; width: 100%;clear: both;">
				<?php echo "<b>" . $newest_page . "</b>"; ?> 
                <select style="font-size: 11px" id="page" onChange="location.href=document.getElementById('page').options[document.getElementById('page').selectedIndex].value">
                    <?php
                        for($x=1;$x<=$pages;$x++){
                            ...
                            if($setting->modrw){
                                echo "<option value=\"new_photos-page" . $x . ".html\" $selected>$x</option>";
                            } else {
                                echo "<option value=\"new_photos.php?page_num=$x\" $selected>$x</option>";
                            }
                        }
                    ?>										
                </select>
                 <?php echo " | "; ?> <?php echo "<b>" . $pages . "</b> " . $newest_pages . " |"; ?> (<strong><?php echo $package_rows; ?></strong> <?php echo $newest_photo; ?>)
                 |
                ...
                <?php
                    if($page_num > 1){
                        if($setting->modrw){
                ?>
                        <a href="new_photos-page<?php echo ($page_num - 1); ?>.html">
                    <?php
                        } else {
                    ?>
                        <a href="new_photos.php?page_num=<?php echo ($page_num-1); ?>">
                    <?php
                        }
                    ?>
                        <?php echo $newest_previous; ?></a>
                <?php
                    } else {
                        echo "$newest_previous"; 
                    }
                    echo " : ";
                    if($page_num != $pages){
                        if($setting->modrw){
		...
		<?
			...
			if($_GET['sgid'] == ""){
				$sub_gallery_result = mysql_query("SELECT id,title FROM photo_galleries where active = '1' and nest_under = '$gid' order by title", $db);
				$sub_gallery_rows = mysql_num_rows($sub_gallery_result);
				if($sub_gallery_rows > 0){
					echo "<hr size=\"1\" class=\"hr\"><span style=\"padding-left: 8px;\"><b><font color=\"#333333\">" . $newest_sub_cat . $current_gallery_name . "</b></span>";
				}		
				while($sub_gallery = mysql_fetch_object($sub_gallery_result)){
		?>
		
					<br><span style="padding-left: 8px;"><a href="new_photos.php?gid=<? echo $_GET['gid']; ?>&sgid=<? echo $sub_gallery->id; ?>" class="sub_gallery_nav"><? echo $sub_gallery->title; ?></a></span>
		<?
				}		
				if($package_rows < 1 and $sub_gallery_rows < 1){
					echo "<span style=\"padding-left: 8px;\">" . $newest_no_photo . "</span>";	
				}
			}
		?>
        <?php
			if(file_exists('rss.php')){
				...
				if($new_feed){
		?>
				<div align="right" style="padding: 5px;"><a href="rss.php?show=new"><img src="images/rss.gif" border="0" /></a></div>
		<?php
				}
			}
		?>
