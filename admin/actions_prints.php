...
/*-----------------------------------------------------------------------------------------------------------------------*/
	/* SAVE NEW ITEM */	
		case "save_new":
			...
			$sql = "INSERT INTO prints (name,article,porder,price,quan_avail,ship_price1,ship_price2,bypass,visible,taxable) VALUES ('$name','$article','$porder','$price','$quan_avail','$ship_price1','$ship_price2','$bypass','$visible','$taxable')";
	...
	/* DELETE ITEM(S) */
		case "delete":
		$result = mysql_query("SELECT id FROM prints", $db);
		while($rs = mysql_fetch_object($result)) {		
			if($_POST[$rs->id] == "1") {
				...
				$sql="DELETE FROM prints WHERE id = '$rs->id'";
				$result2 = mysql_query($sql);
			}
		}
	...
	/* SAVE EDIT ITEM */	
		case "save_edit":
			...
			$publish_date = $s_year . $s_month . $s_day;	
			$sql = "UPDATE prints SET name='$name',article='$article',porder='$porder',price='$price',quan_avail='$quan_avail',ship_price1='$ship_price1',ship_price2='$ship_price2',bypass='$bypass',visible='$visible',taxable='$taxable' WHERE id = '$item_id'";
			$result = mysql_query($sql);
