...
if ($vote_sent > $units) die($rating_sorry_invalid); // kill the script because normal users will never see this.
...
$new_back[] .= '<li class="current-rating" style="width:'.@number_format($current_rating/$count,2)*$rating_unitwidth.'px;">' . $rating_current_rating . '</li>';
...
$new_back[] .= '</ul>';
$new_back[] .= '<p class="voted">' . $rating_rating . '<strong>'.@number_format($sum/$added,1).'</strong>/'.$units.' ('.$count.' '.$tense.' '.$rating_cast.') ';
$new_back[] .= '<span class="thanks">' . $rating_thanks . '</span></p>';
